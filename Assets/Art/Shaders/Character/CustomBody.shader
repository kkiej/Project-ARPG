Shader "Custom/Body"
{
	Properties
	{
		...
		_Mask("Mask", 2D) = "white" {}
		_MaskTex_2("Mask Tex_2", 2D) = "white" {}
	...
	SubShader
	{
		...
		Pass
		{
			...
			inline half DepthRim(half depthRimOffset, half rimDepthDiffThresholdOffset, half2 positionCSXY, half3 mainLightDir, float linearEyeDepth)
			{
			    ...
			    depthRim = saturate(lerp(0, depthRim, linearEyeDepth));
			    return depthRim;
			}
				 ...
				 ) : SV_Target
			{
				...
				float4 MetalSpecular = Mask.a * SAMPLE_TEXTURE2D(_Metal, sampler_Metal, uv_MatCap * 0.5 + 0.5);
				...
				float2 uv_MaskTex_2 = IN.uv * _MaskTex_2_ST.xy + _MaskTex_2_ST.zw;
				float4 Mask2 = SAMPLE_TEXTURE2D(_MaskTex_2, sampler_MaskTex_2, uv_MaskTex_2);
				...
				//RimColor = DepthRim(_RimOffset, _RimThreshold, IN.positionCS.xy, mainLight.direction, linearEyeDepth) * _RimColor;
				float3 Color = saturate((MetalSpecular + diffuse * Mask2.g * Mask2.r * 10.0 * BaseColor + Specular + RimColor) * mainLight.color * mainLight.distanceAttenuation).rgb;
