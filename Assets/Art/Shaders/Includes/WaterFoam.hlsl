...
void FoamSample(half4 uvs, half2 sampling, sampler2D tex, half2 shadowoffset, out half4 Out)
{
    half cutoff = sampling.x;
    half distortion = sampling.y;
    half2 uv1 = uvs.xy;
    ...
    half foam1 =   saturate(step(cutoff, tex2D(tex, DistortUV(uv1,                distortion)).r));
    half foam2 =   saturate(step(cutoff, tex2D(tex, DistortUV(uv2,                distortion)).g));

    half shadow1 = saturate(step(cutoff, tex2D(tex, DistortUV(uv1 + shadowoffset, distortion)).r));
    half shadow2 = saturate(step(cutoff, tex2D(tex, DistortUV(uv2 + shadowoffset, distortion)).g));
