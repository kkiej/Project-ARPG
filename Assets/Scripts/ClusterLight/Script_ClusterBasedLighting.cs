...
public class Script_ClusterBasedLighting : MonoBehaviour
{
    ...
    public ComputeShader    cs_AssignLightsToClusters;
    ...
    public Material         mtl_DepthPrePass;
    ...
    void Pass_ComputeClusterAABB()
    {
        ...
        cs_ComputeClusterAABB.SetMatrix("_InverseProjectionMatrix", projectionMatrixInverse);
    ...
    void Pass_AssignLightsToClusters()
    {
        ...
        int kernel = cs_AssignLightsToClusters.FindKernel("CSMain");

        //Output
        cs_AssignLightsToClusters.SetBuffer(kernel, "RWPointLightIndexCounter_Cluster", cb_ClusterPointLightIndexCounter);
        cs_AssignLightsToClusters.SetBuffer(kernel, "RWPointLightGrid_Cluster", cb_ClusterPointLightGrid);
        cs_AssignLightsToClusters.SetBuffer(kernel, "RWPointLightIndexList_Cluster", cb_ClusterPointLightIndexList);

        //Input
        cs_AssignLightsToClusters.SetInt("PointLightCount", lst_Light.Count);
        cs_AssignLightsToClusters.SetMatrix("_CameraLastViewMatrix", _camera.worldToCameraMatrix);
        cs_AssignLightsToClusters.SetBuffer(kernel, "PointLights", cb_PointLightPosRadius);
        cs_AssignLightsToClusters.SetBuffer(kernel, "ClusterAABBs", cb_ClusterAABBs);
        cs_AssignLightsToClusters.SetBuffer(kernel, "UniqueClusters", cb_UniqueClusters);

        //cs_AssignLightsToClusters.Dispatch(kernel, m_DimData.clusterDimXYZ, 1, 1);
        cs_AssignLightsToClusters.DispatchIndirect(kernel, cb_IAB_AssignLightsToClusters);
    }

    void Pass_DepthPre()
    {
        mtl_DepthPrePass.SetPass(0);
        DrawMeshListNow();
    }
    ...
    void Pass_ClusterSample_CSVer()
    {
        ...
        cs_ClusterSample.SetTexture(kernel, Cluster_ShaderIDs.DepthTexture, _rtDepth);
        cs_ClusterSample.SetBuffer(kernel, Cluster_ShaderIDs.RWClusterFlags, cb_ClusterFlag);
        cs_ClusterSample.SetMatrix(Cluster_ShaderIDs.InverseProjectionMatrix, projectionMatrixInvers);        
        cs_ClusterSample.SetVector(Cluster_ShaderIDs.ClusterCB_ScreenDimensions, screenDim);
    ...
    void Pass_DebugLightCount()
    {
        ...
        var projectionMatrix = GL.GetGPUProjectionMatrix(_camera.projectionMatrix, true);
        var projectionMatrixInvers = projectionMatrix.inverse;
        cs_DebugLightCount.SetMatrix(Cluster_ShaderIDs.InverseProjectionMatrix, projectionMatrixInvers);
    ...
    void UpdateClusterCBuffer(ComputeShader cs)
    {
        ...
        cs.SetInts(Cluster_ShaderIDs.ClusterCB_GridDim, gridDims);        
        cs.SetInts(Cluster_ShaderIDs.ClusterCB_Size, sizes);
        cs.SetFloat(Cluster_ShaderIDs.ClusterCB_ViewNear, viewNear);
        cs.SetFloat(Cluster_ShaderIDs.ClusterCB_NearK, 1.0f + m_DimData.sD);
        cs.SetFloat(Cluster_ShaderIDs.ClusterCB_LogGridDimY, m_DimData.logDimY);
        cs.SetVector(Cluster_ShaderIDs.ClusterCB_ScreenDimensions, screenDim);
    }
