    ...
    [Serializable] internal class FR2_AssetDB
    {
        ...
        internal void BuildCache()
        {
            ...
            for (var i = 0; i < files.Count; i++)
            {
                ...
                // OPTIMIZED: Build fileIdMap in single O(n) pass instead of O(n²) IndexOf
                assetFile.fileIdMap.Clear();
                for (var j = 0; j < assetFile.fileIds.Count; j++)
                {
                    long fileId = assetFile.fileIds[j];
                    if (!assetFile.fileIdMap.ContainsKey(fileId))
                    {
                        assetFile.fileIdMap.Add(fileId, j);
                    }
                }
        ...
        internal void TS_ReadFileContent(int index)
        {
            ...
            string assetPath = FR2_Cache.GUIDToAssetPath(sourceFile.guid);
