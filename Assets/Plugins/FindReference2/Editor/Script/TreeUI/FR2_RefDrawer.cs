...
namespace vietlabs.fr2
{
    internal partial class FR2_RefDrawer : IRefDraw
    {
        ...
        public FR2_RefDrawer Reset(GameObject[] objs, bool findDept, bool findPrefabInAsset)
        {
            ...
            var guidsSet = new HashSet<string>(); // For O(1) Contains() lookup
            Dictionary<GameObject, HashSet<string>> dependent = FR2_SceneCache.Api.prefabDependencies;
            
            for (var i = 0; i < objs.Length; i++)
            {
                if (!dependent.TryGetValue(objs[i], out HashSet<string> hash)) continue;
                foreach (string guid in hash)
                {
                    if (!guidsSet.Add(guid)) continue;
                    guidss.Add(guid);
                }
            }
            ...
            foreach (KeyValuePair<string, FR2_Ref> kvp in usageRefs1)
            {
                ...
                if (guidsSet.Contains(kvp.Key)) kvp.Value.depth = 1;
            ...
            if (findPrefabInAsset)
            {
                ...
                var prefabGuidsSet = new HashSet<string>(); // For O(1) Contains() lookup
                ...
                for (var i = 0; i < objs.Length; i++)
                {
                    ...
                    if (prefabGuidsSet.Add(guid)) // Add returns false if already exists
                    {
                        guids.Add(guid);
                    }
                }
                ...
                foreach (KeyValuePair<string, FR2_Ref> kvp in usageRefs)
                {
                    ...
                    if (prefabGuidsSet.Contains(kvp.Key)) kvp.Value.depth = 1;
        ...
        {
            FR2_LOG.Log(nameof(ResetSceneInScene));
        ...
        {
            FR2_LOG.Log(nameof(ResetSceneUseSceneObjects));
