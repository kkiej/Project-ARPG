...
namespace vietlabs.fr2
{
    ...
    partial class FR2_Cache
    {
        ...
        {
            if (FR2_SettingExt.disable) { EditorApplication.update -= CheckDelayRefresh; return; }
        ...
        {
            if (guidToPathCache.TryGetValue(guid, out string cachedPath))
            {
                return cachedPath;
            }
        ...
        {
            if (instanceIdStringCache.TryGetValue(instanceId, out string cached))
            {
                return cached;
            }
    ...
    internal partial class FR2_Cache : ScriptableObject
    {
        ...
        [NonSerialized] internal static readonly Dictionary<string, string> guidToPathCache = new Dictionary<string, string>();
        [NonSerialized] internal static readonly Dictionary<int, string> instanceIdStringCache = new Dictionary<int, string>();
        [NonSerialized] internal static readonly Dictionary<int, (string guid, long localId)> instanceIdToGuidLocalIdCache = new Dictionary<int, (string, long)>();
        ...
        private static void RefreshDB()
        {
            ...
            MarkDirty();
            // Debug.Log($"HasDirtyAsset: {hasDirtyAsset}");
        ...
        {
            if (!MatchesStatus(nameof(SetAsReady), FR2_Status.InitCacheMap, FR2_Status.BuildUsedByMap)) return;
