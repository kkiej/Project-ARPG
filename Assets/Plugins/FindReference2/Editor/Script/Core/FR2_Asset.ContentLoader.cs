...
namespace vietlabs.fr2
{
    internal partial class FR2_Asset
    {
        ...
        internal void ValidateWithAssetDatabase()
        {
            ...
            try
            {
                var unityDeps = AssetDatabase.GetDependencies(assetPath, false);
                if (unityDeps == null) return;
                var unityGuids = new HashSet<string>();
                
                foreach (var depPath in unityDeps)
                {
                ...
                foreach (var unityGuid in unityGuids)
                {
                    if (fr2Guids.Contains(unityGuid)) continue;
